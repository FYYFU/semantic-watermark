...
class DataTrainingArguments:
    ...
    save_sentence_matrix: bool = field(
        default=False, metadata={"help": "whether to save sentence matrix. (if choice plz set do_train = True1)"}
    )
    sentence_matrix_path: str = field(
        default=None, metadata={"help": "path to save sentence matrix."}
    )
    ...
    detect_watermark: bool = field(
        default=None,
        metadata={"help": "whether to detect watermark"}
    )
    show_generated_result: bool = field(
        default=None, 
        metadata={"help": "whether to show generated detect result."}
    )
    show_reference_result: bool = field(
        default=None, 
        metadata={"help": "whether to show reference detect result."}
    )
...
def main():
    ...
    if data_args.save_similar_matrix:
        ...
        word_vector = word_vector[:embedding_size, :]
        ...
        for i in tqdm(range(num_batches)):
            ...
            _, indices = torch.sort((norm_query.reshape(-1, 1) + norm_vector - 2 * query @ word_vector.T), dim=1, descending=False)
    ...
    if training_args.do_predict:
        ...
        if data_args.detect_watermark:
            ...
            if data_args.use_cluster_enhance:
                setattr(watermark_detector, 'similar_matrix', similar_matrix)
                setattr(watermark_detector, 'cluster_k', data_args.watermark_cluster_k)
            if trainer.is_world_process_zero():
                if data_args.show_generated_result:
                    predictions = tokenizer.batch_decode(
                        predict_results.predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True
                    )
                    predictions = [pred.strip() for pred in predictions]
                    save_path = os.path.join(training_args.output_dir, "detect_information.txt")
                    writer = open(save_path, 'w') 
                    for index, line in tqdm(enumerate(predictions), total=len(predictions)):
                        score_dict = watermark_detector.detect(text=line.strip(), input_tokenize_text=predict_dataset[index]['input_ids'])
                        writer.write(json.dumps(score_dict) + '\n')
                    writer.close()
                elif data_args.show_reference_result:
                    with training_args.main_process_first(desc="prediction dataset map pre-processing"):
                        predict_dataset = predict_dataset.map(
                            preprocess_function,
                            batched=True,
                    ...
                    save_path = os.path.join(training_args.output_dir, "detect_information_reference.txt")                
                    with open(save_path, 'w') as writer:
                        for example in tqdm(predict_dataset, total=len(predict_dataset)):
                            line = tokenizer.decode(example['labels'], skip_special_tokens=True).strip()
                            score_dict = watermark_detector.detect(text=line, input_tokenize_text=example['input_ids'])
                            writer.write(json.dumps(score_dict) + '\n')
                else:
                    raise ValueError("set either show_generated_result=True or show_reference_result=True")
        #! detect watermark
